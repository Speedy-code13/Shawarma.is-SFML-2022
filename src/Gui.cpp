...
gui::Button::Button(float x	, float y, float width, float height, sf::Font* font, std::string text,unsigned characterSize, 
...
gui::DropDownList::DropDownList(float x, float y, float width, float height, sf::Font& font, std::string list[], unsigned number_of_elements, unsigned default_index) 
	: font(font), showList(false), keytimeMax(10.f), keytime(keytimeMax), numberOfElements(number_of_elements), height(height)
{	
...
void gui::DropDownList::setActiveElementId(const unsigned short id)
{
	this->activeElement->setId(id);
}

const bool gui::DropDownList::getKetime()
{
...
const sf::Vector2f gui::DropDownList::getSize() const
{
	return this->activeElement->getSize();
}

void gui::DropDownList::setPosition(float x, float y)
{
	this->activeElement->setPosition(x, y);
	for (size_t i = 0; i < this->numberOfElements; i++)
	{
		this->list[i]->setPosition(x, y + (i+1) * height);
	}
}
...
void gui::DropDownList::update(sf::Vector2f& mousePos, const float& dt)
{
	...
	if (this->activeElement->isPressed() && this->getKetime())
	{
...
gui::TextureSelector::TextureSelector(float x, float y, float width, float height, float grid_size, const sf::Texture* texture_sheet)
{
	...
	this->bounds.setOutlineColor(sf::Color(255, 255, 255, 100));
	...
	this->selector.setSize({ this->gridSize, this->gridSize });
